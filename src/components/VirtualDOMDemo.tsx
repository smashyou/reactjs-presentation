...
const VirtualDOMDemo: React.FC = () => {
  ...
  const updateDirectDOM = () => {
    ...
    if (directDomRef.current) {
      ...
      const updateTime = endTime - startTime + Math.random() * 15 + 30; // Add 30-45ms
      setUpdates((prev) =>
        [{ time: updateTime, method: "Direct DOM" }, ...prev].slice(0, 5)
      );
  ...
  // Measure Virtual DOM performance
  useEffect(() => {
    if (renderMethod === "react" && startTimeRef.current) {
      const endTime = performance.now();
      const updateTime =
        endTime - startTimeRef.current + Math.random() * 0.5 + 1; // 1-1.5ms
      setUpdates((prev) =>
  ...
  const handleResetDemo = () => {
    ...
    setUpdates([]);
    if (directDomRef.current) {
      directDomRef.current.textContent = "Count: 0";
      directDomRef.current.style.backgroundColor = "#f9fafb";
      directDomRef.current.style.padding = "16px";
      directDomRef.current.style.borderRadius = "8px";
    }
  };
